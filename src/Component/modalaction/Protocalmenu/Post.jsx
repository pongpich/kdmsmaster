...
function Post(props) {
  ...
  const [tmltyp, setTmltyp] = useState("PTV");
  ...
  const [ptvList, setPtvList] = useState("");
  const [Arrlist, setArrlist] = useState([]);
  const [typcod, setTypcod] = useState("I");
  const utf8 = require("utf8");
  const [PTV, setPtv] = useRecoilState(PTVstate);
  const [count, setCount] = useState("1");
  const [PTVresult, setPTVresult] = useState(props.data);
  ...
  useEffect(() => {
    console.log(props.data, "TestDataList");
    console.log(props.test, "TestData");
    Filter();
  }, [PTVresult]);
  ...
  const GETPOST = async () => {
    ...
    try {
      ...
      setPtvList(data.result);
      console.log(ptvList, "ptvList after await");
  ...
  const Filter = () => {
    if (PTVresult) {
      const PTVResult = PTVresult.filter((data) => {
        return data.TypTopic === "PTV                 ";
      });
      PTVResultdata.push(PTVResult);
      setDataResult(PTVResultdata);
      console.log(PTVResultdata, "dataTest");
    }
  };
  ...
  const SavePost = async () => {
    ...
    // const SetSeq = useRecoilValue(PTVSetstate.SetSeq) + 1;
    // setPtv([{ SetTyp: tmltyp.trim(), SetPrd: count, DtlCod: tmlcod.trim() }]);

    // setPtv([{tmlcod: tmlcod, tmltyp: tmltyp,namtha: namtha,nameng: nameng,dsctha:dsctha ,dsceng:dsceng}])

    // setPtv([{ SetTyp: tmltyp, SetPrd: 1, SetSeq: SetSeq, DtlCod: tmlcod }]);
    SaveDetail();
  };
  ...
  const SetDataArr = useEffect(() => {
    ...
    if (Arrlist.TmlCod !== undefined && tmlcod !== undefined) {
      const filter = ptvList.some((data) => {
        return data.TmlCod === tmlcod;
      });
      if (filter === true) {
        setTypcod("U");
      }

      console.log(filter, "ดูหน่อย ");
      console.log(typcod);
    } else {
      setTmlcod("");
      setNamtha("");
      ...
      <div className="grid grid-cols-12">
        ...
        <div className="col-span-11 font-medium">
          ...
          <div className="text-xs text-neutral-500  font-semibold ">
            PW(1-2) <span className=" text-blue-800 underline">Hide All</span>
          </div>
        </div>
      </div>
      <div className="grid grid-cols-12">
        ...
        <div className="col-span-11 font-medium">
          ...
          {PTVresult?.length > 0 ? (
            <>
              {PTVresult.filter((name) =>
                name.TypTopic.includes("PTV                 ")
              ).map((i) => (
